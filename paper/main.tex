...
\paragraph{Keywords.}
neuroscience-inspired AI, cerebellum, online learning, AI agent safety, tool-call evaluation, forward model, LLM efficiency, somatic marker, metacognition, intrinsic motivation
...
\begin{itemize}[nosep]
  \item A principled mapping from cerebellar neuroscience to AI agent architecture, grounded in the UCT hypothesis, extending across three phases: core engine, signal processing, and emergent cognition (\Cref{sec:method}).
  \item An open-source implementation featuring Random Fourier Feature expansion, K-head population coding with dendritic masking, three biologically-motivated error channels, adaptive confidence-gated routing, frequency filtering, Golgi feedback gating, state estimation, somatic markers (intuition), curiosity-driven exploration, and metacognitive self-awareness (\Cref{sec:implementation}).
  \item Comprehensive evaluation: 97.0\% accuracy with Phase~3 components; $175\times$ speedup; ablation studies quantifying each component; a closed-loop distillation experiment; and a temporal-pattern benchmark showing state-conditioning improves accuracy from 69.5\% to 79.7\% (\Cref{sec:experiments}).
\end{itemize}
...
The complete architecture consists of $\sim$4,000 lines of code (109 tests) with no GPU requirement---all inference and training run on CPU in real time.
...
Immediate directions include (1)~evaluation on real-world agent traces (SWE-bench, WebArena), (2)~a MOBA game microzone to demonstrate micro-macro cognitive splitting, (3)~formal convergence analysis under non-stationary distributions, and (4)~integration with the MCP protocol for plug-and-play deployment in any AI agent framework.
